...
def start():
    ...
    if a.get() != "q":
        ...
        choice=messagebox.askyesno("Exit ?","Do you want to leave the dice Roller ?")
        if choice:
            root.destroy()
def roll_dice(number):
    ...
    for i in range(number):
        ...
        widget_references.append(lab)
        lab.grid(row=i+3,column=3,pady=5)
        ...
        widget_references.append(result)
...
mainhead.grid(row=0,column=0,pady=10)
head = Label(root,text='''• Enter number of dices to be roll or enter "q" to quit rolling
• Do not enter more than 15 number''',fg="grey",font="detail_font")
head.grid(row=1,column=1,columnspan=3,pady=10)
